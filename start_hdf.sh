...
do_site2site_configure() {
  ...
  sed -i "s/nifi\.remote\.input\.socket\.port=.*/nifi.remote.input.socket.port=2881/g"  ${HDF_HOME}/conf/nifi.properties
...
do_cluster_node_configure() {
  ...
  sed -i "s/nifi\.cluster\.node\.protocol\.port=.*/nifi.cluster.node.protocol.port=2882/g"  ${HDF_HOME}/conf/nifi.properties
  ...
  srv=1; IFS=","; for node in $ZK_NODES; do sed -i "\$aserver.$srv=$node:${ZK_INTERCOM_PORT}:${ZK_ELECTION_PORT}"  ${HDF_HOME}/conf/zookeeper.properties; let "srv+=1"; done
}
